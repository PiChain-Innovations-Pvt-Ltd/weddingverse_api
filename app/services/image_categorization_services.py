...
async def categorize_and_match(
    ...
    location:         Optional[str] = None,  # ADDED: location parameter
    limit:            int = 10
) -> Dict[str, Any]:  # FIXED: Return type should be Dict, not List

    # FIXED: Initialize output outside the loop to avoid UnboundLocalError
    output = None
    ...
    for img_bytes, content_type in zip(upload_bytes_list, content_types):
        try:
            ...
            # FIXED: Prepare prompts with location included
            vb_req = VisionBoardRequest(
                reference_id=      reference_id,
                wedding_preference=cleaned["wedding_preference"],
                venue_suits=       cleaned["venue_suits"],
                ...
                guest_experience=  cleaned["guest_experience"],
                events=            cleaned["events"],
                location=          location  # FIXED: Include location parameter
            )
            user_input   = json.dumps(vb_req.dict(), indent=2)
            system_prompt = (
            "You are a specialized AI assistant for processing wedding vision board inputs. "
            ...
            # FIXED: Break after successful processing of first image
            break
    ...
    # FIXED: Handle case where no images were successfully processed
    if output is None:
...
async def categorize_bulk(
    ...
    # FIXED: Include location in VisionBoardRequest
    vb_req = VisionBoardRequest(
        reference_id=      reference_id,
        wedding_preference=cleaned["wedding_preference"],
        venue_suits=       cleaned["venue_suits"],
